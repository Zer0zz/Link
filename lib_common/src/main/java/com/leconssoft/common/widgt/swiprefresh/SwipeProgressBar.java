 ...
 * trigger a refresh).
 */
final class SwipeProgressBar extends BaseSwipProgressBar {
    ...
    private final static int REFRESH_MSG_MARGINTOP = 120;
...
//        final int cy = height / 2;
        ...
        if (mRunning || (mFinishTime > 0)) {
            ...
            long elapsed = (now - mStartTime) % ANIMATION_DURATION_MS_NORMAL;
            long iterations = (now - mStartTime) / ANIMATION_DURATION_MS_NORMAL;
            float rawProgress = (elapsed / (ANIMATION_DURATION_MS_NORMAL / 100f));
    ...
    private void drawMsg(Canvas canvas, String msg){
        textPaint.setTextSize(38);
        textPaint.setColor(0xB3000000);
        float textWidth = textPaint.measureText(msg);
        canvas.drawText(msg, (mBounds.width() - textWidth)/2, mBounds.height() + REFRESH_MSG_MARGINTOP, textPaint);
    }
